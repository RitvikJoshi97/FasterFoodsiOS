...
struct ShoppingListView: View {
    ...
    var body: some View {
        ...
        .confirmationDialog("Delete List?", isPresented: Binding(
            ...
            Text("This will remove the entire list and all of its items.")
        }
        .toolbar {
    ...
    private var shoppingContent: some View {
        if isLoading && app.shoppingLists.isEmpty {
            loadingState
        } else if app.shoppingLists.isEmpty {
            emptyState
        } else {
            listsView
        }
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("Create your first list")
                .font(.headline)
            Text("Get started by naming a list and adding a few staples.")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
            standaloneNewListForm
                .padding()
                ...
                if shouldShowQuickAddDetails {
                    ...
                    Picker("List", selection: $selectedListId) {
                        if app.shoppingLists.isEmpty {
                            Text("Select a list").tag(newListSentinel)
                        }
                        ForEach(app.shoppingLists) { list in
    ...
    private var standaloneNewListForm: some View {
        VStack(alignment: .leading, spacing: 12) {
            TextField("New list name", text: $newListName)
                .focused($focusedField, equals: .newListName)
            Button {
                Task { await submitNewList() }
            } label: {
                if isCreatingList {
                    ProgressView()
                } else {
                    Label("Create List", systemImage: "text.badge.plus")
                        .frame(maxWidth: .infinity)
                }
            }
            .buttonStyle(.borderedProminent)
            .disabled(newListName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty || isCreatingList)
        }
    }

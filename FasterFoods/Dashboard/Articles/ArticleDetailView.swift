...
struct ArticleDetailView: View {
    ...
    private var heroImageOverlay: some View {
        LinearGradient(
            colors: [
    ...
    private func loadArticleContent() async {
        ...
        let possibleURLs = [
            Bundle.main.url(
                forResource: article.markdownResourceName, withExtension: "md",
...
struct ArticleReferencePreprocessor {
    ...
    private static func extractFootnotes(from markdown: String) -> [String: URL] {
        ...
        for line in lines {
            guard line.hasPrefix("["), let closingIndex = line.firstIndex(of: "]"),
                line.contains("]:")
            else {
                continue
            }
    ...
    private static func replaceReferencePlaceholders(
        in text: String,
        footnotes: [String: URL]
    ) -> Result {
        ...
        for match in matches {
            ...
            let title = nsText.substring(with: match.range(withName: "title")).trimmingCharacters(
        ...
        let cleaned = resultsText.replacingOccurrences(
            of: residualPattern, with: "", options: .regularExpression)
        return Result(cleanedMarkdown: cleaned, references: references)
    }
}

extension String {
    fileprivate func matches(regex: String) -> Bool {
...
struct ArticleMarkdownView: View {
    ...
    init(
        markdownText: String, references: [String: MarkdownLinkReference],
                        ...
                        ForEach(Array(items.enumerated()), id: \.offset) { entry in
                            HStack(alignment: .top, spacing: 8) {
                                Circle()
                                    ...
                                    .padding(.top, 6)
                                InlineMarkdownText(
                                    entry.element, articles: articleTopics, references: references
                                )
                                .font(.body)
                                .frame(maxWidth: .infinity, alignment: .leading)
                            }
    ...
    static func parse(_ markdown: String) -> [ArticleBlock] {
        ...
        for rawLine in lines {
            ...
            if let headingLevel = headingLevel(for: trimmed) {
                flushParagraph()
                flushBulletList()
                let textStart = trimmed.index(
                    trimmed.startIndex, offsetBy: min(headingLevel, trimmed.count))
...
struct InlineMarkdownText: View {
    ...
    init(_ text: String, articles: [ArticleTopic], references: [String: MarkdownLinkReference]) {
        self.segments = InlineSegmentParser.segments(
            from: text, articles: articles, references: references)
    }
...
enum InlineSegmentParser {
    ...
    private static func reference(for url: URL, fallbackTitle: String, articles: [ArticleTopic])
    ...
    private static func resolveArticle(from path: String, within articles: [ArticleTopic])
        -> ArticleTopic?
    {
    ...
    private static func tokenizeWord(
        ...
        guard let (key, reference) = references.first(where: { plain.contains($0.key) }),
            ...
            let upper = AttributedString.Index(plainRange.upperBound, within: word)
        else {

...
actor APIClient {
    ...
    var baseURL: URL {
        let envValue = ProcessInfo.processInfo.environment["API_BASE_URL"]?.trimmingCharacters(
    ...
    private func request(
        ...
        if let absoluteURL = URL(string: path), absoluteURL.scheme != nil {
            ...
            guard let resolved = URL(string: normalized, relativeTo: baseURL)?.absoluteURL else {
                throw APIError(
                    statusCode: nil, message: "Invalid URL path: \(path)", unverified: false,
        ...
        if authorized {
            guard let token, !token.isEmpty else {
                throw APIError(
                    statusCode: nil, message: "Authentication required. Please log in again.",
                    unverified: false, isNetworkError: false)
            }
        ...
        do {
            ...
            throw APIError(
                statusCode: nil, message: error.localizedDescription, unverified: false,
                isNetworkError: isNetworkError)
        }
        guard let http = response as? HTTPURLResponse else {
            throw APIError(
                statusCode: nil, message: "Invalid server response.", unverified: false,
                isNetworkError: false)
        }
        guard (200..<300).contains(http.statusCode) else {
            ...
            if let errorPayload = try? JSONDecoder().decode(APIErrorResponse.self, from: data) {
                if let joined = errorPayload.details {
                    ...
                    message =
                        errorPayload.error ?? errorPayload.message ?? errorPayload.detail
            ...
            throw APIError(
                statusCode: http.statusCode, message: message, unverified: unverifiedFlag,
                isNetworkError: false)
        }
        return (data, http)
    }
    ...
    func validateToken() async -> TokenValidationResult {
        guard hasToken() else { return .invalid }
        do {
            _ = try await request("/validate", authorized: true)
            return .valid
        } catch let apiError as APIError where apiError.isNetworkError {
            return .networkError
        } catch {
            return .invalid
        }
    ...
    func register(email: String, password: String, firstName: String, lastName: String) async throws
    {
        ...
        let (_, http) = try await request(
            "/register", method: "POST", body: body, contentType: "application/json")
    ...
    func resendVerification(email: String) async throws {
        ...
        let (_, http) = try await request(
            "/resend-verification", method: "POST", body: body, contentType: "application/json")
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
    }
    ...
    func login(email: String, password: String) async throws -> LoginResponse {
        ...
        let (data, http) = try await request(
            "/login", method: "POST", body: body, contentType: "application/json")
        guard (200..<300).contains(http.statusCode) else {
    ...
    func loginWithApple(
        ...
        let body = try JSONSerialization.data(withJSONObject: payload)
        let (data, http) = try await request(
            "/auth/apple", method: "POST", body: body, contentType: "application/json")
        guard (200..<300).contains(http.statusCode) else {
            throw URLError(.userAuthenticationRequired)
        }
        let res = try JSONDecoder().decode(LoginResponse.self, from: data)
    ...
    func loginWithGoogle(
        ...
        let body = try JSONSerialization.data(withJSONObject: payload)
        let (data, http) = try await request(
            "/auth/google", method: "POST", body: body, contentType: "application/json")
        guard (200..<300).contains(http.statusCode) else {
            throw URLError(.userAuthenticationRequired)
        }
        let res = try JSONDecoder().decode(LoginResponse.self, from: data)
    ...
    func getSettings() async throws -> UserSettings {
        ...
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
        return try JSONDecoder().decode([String: UserSettings].self, from: data)["settings"]
            ?? UserSettings()
    }
    ...
    func updateSettings(_ settings: UserSettings) async throws -> UserSettings {
        ...
        let (data, http) = try await request(
            "/settings", method: "PUT", body: body, contentType: "application/json",
            authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
        return try JSONDecoder().decode([String: UserSettings].self, from: data)["settings"]
            ?? settings
    ...
    func uploadImage(_ data: Data, forPath path: String) async throws -> Data {
        ...
        body.append(data)
        body.append("\r\n--\(boundary)--\r\n".data(using: .utf8)!)
        let (respData, http) = try await request(
            path, method: "POST", body: body,
            contentType: "multipart/form-data; boundary=\(boundary)", authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
        return respData
    }
    ...
    func changePassword(current: String, new: String) async throws {
        ...
        let body = try JSONSerialization.data(withJSONObject: payload)
        let (_, http) = try await request(
            "/change-password", method: "POST", body: body, contentType: "application/json",
            authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
    }
    ...
    func forgotPassword(email: String) async throws {
        ...
        let body = try JSONSerialization.data(withJSONObject: payload)
        let (_, http) = try await request(
            "/forgot-password", method: "POST", body: body, contentType: "application/json")
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
    }
    ...
    func resetPassword(token: String, newPassword: String) async throws {
        ...
        let body = try JSONSerialization.data(withJSONObject: payload)
        let (_, http) = try await request(
            "/reset-password", method: "POST", body: body, contentType: "application/json")
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
    }
    ...
    func generateInvite() async throws -> (code: String, expiry: String) {
        let (data, http) = try await request(
            "/user/generate-family-invite", method: "POST", authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
    ...
    func joinFamily(code: String) async throws -> String {
        ...
        let (data, http) = try await request(
            "/user/join-family", method: "POST", body: body, contentType: "application/json",
            authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
        let obj = try JSONSerialization.jsonObject(with: data) as? [String: Any]
    ...
    func verifySharedItem(id: Int, accepted: Bool) async throws {
        ...
        let (_, http) = try await request(
            "/verify-shared-item/\(id)", method: "POST", body: body,
    ...
    func shareItem(itemId: Int, userIds: [Int]) async throws {
        let body = try JSONSerialization.data(withJSONObject: [
            "itemId": itemId, "sharedUserIds": userIds,
        ])
        let (_, http) = try await request(
            "/share-item", method: "POST", body: body, contentType: "application/json",
            authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
    }
    ...
    func createShoppingList(name: String) async throws -> ShoppingList {
        ...
        let (data, http) = try await request(
            "/shopping-lists", method: "POST", body: body, contentType: "application/json",
            authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
        return try JSONDecoder().decode(ShoppingList.self, from: data)
    }
    ...
    func deleteShoppingList(id: String) async throws {
        let (_, http) = try await request(
            "/shopping-lists/\(id)", method: "DELETE", authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
    }

    func addShoppingItem(
        toList listId: String, name: String, quantity: String? = nil, unit: String? = nil,
        ...
        let body = try JSONSerialization.data(withJSONObject: payload)
        let (data, http) = try await request(
            "/shopping-lists/\(listId)/items", method: "POST", body: body,
            contentType: "application/json", authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
        return try JSONDecoder().decode(ShoppingItem.self, from: data)
    }

    func updateShoppingItem(listId: String, itemId: String, updates: [String: Any]) async throws
        -> ShoppingItem
    {
        let body = try JSONSerialization.data(withJSONObject: updates)
        let (data, http) = try await request(
            "/shopping-lists/\(listId)/items/\(itemId)", method: "PATCH", body: body,
            contentType: "application/json", authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
        return try JSONDecoder().decode(ShoppingItem.self, from: data)
    }
    ...
    func deleteShoppingItem(listId: String, itemId: String) async throws {
        let (_, http) = try await request(
            "/shopping-lists/\(listId)/items/\(itemId)", method: "DELETE", authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
    }
    ...
    func sendShoppingRecommendationFeedback(id: String, action: String) async throws {
        ...
        let (_, http) = try await request(
            "/recommendations/\(id)/feedback", method: "POST", body: body,
            contentType: "application/json", authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
    }
    ...
    func getPantryItems() async throws -> [PantryItem] {
        ...
        throw APIError(
            statusCode: nil, message: "Unexpected pantry response", unverified: false,
            isNetworkError: false)
    }

    func createPantryItem(name: String, quantity: String?, unit: String?, expiryDate: String?)
        async throws -> PantryItem
    {
        ...
        let body = try JSONSerialization.data(withJSONObject: payload)
        let (data, http) = try await request(
            "/pantry/items", method: "POST", body: body, contentType: "application/json",
            authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
        let decoder = JSONDecoder()
    ...
    func updatePantryItem(id: String, updates: [String: Any]) async throws -> PantryItem {
        let body = try JSONSerialization.data(withJSONObject: updates)
        let (data, http) = try await request(
            "/pantry/items/\(id)", method: "PUT", body: body, contentType: "application/json",
            authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
        let decoder = JSONDecoder()
        if let item = try? decoder.decode(PantryItem.self, from: data) {
    ...
    func togglePantryItem(id: String) async throws -> PantryItem {
        let (data, http) = try await request(
            "/pantry/items/\(id)/toggle", method: "PATCH", authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
    ...
    func getFoodLogItems() async throws -> [FoodLogItem] {
        ...
        if let wrapped = try? decoder.decode([String: [FoodLogItem]].self, from: data),
            let items = wrapped["items"]
        {
            return items
        }
        if let wrappedResponse = try? decoder.decode([String: FoodLogResponse].self, from: data),
            let response = wrappedResponse.values.first
        {
        ...
        if let single = try? decoder.decode([String: FoodLogItem].self, from: data),
            let item = single.values.first
        {
            return [item]
        }
        throw APIError(
            statusCode: nil, message: "Unexpected food log response", unverified: false,
            isNetworkError: false)
    }
    ...
    func createFoodLogItem(_ requestBody: FoodLogCreateRequest) async throws -> FoodLogItem {
        ...
        let (data, http) = try await request(
            "/food-log/items", method: "POST", body: body, contentType: "application/json",
            authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
        let decoder = JSONDecoder()
        ...
        if let wrappedResponse = try? decoder.decode([String: FoodLogResponse].self, from: data),
            let response = wrappedResponse.values.first, let first = response.items.first
        {
            return first
        }
        if let response = try? decoder.decode(FoodLogResponse.self, from: data),
    ...
    func getWorkoutItems() async throws -> [WorkoutLogItem] {
        ...
        throw APIError(
            statusCode: nil, message: "Unexpected workout response", unverified: false,
            isNetworkError: false)
    }
    ...
    func createWorkoutItem(_ request: WorkoutCreateRequest) async throws -> WorkoutLogItem {
        ...
        let (data, http) = try await self.request(
            "/workout/items", method: "POST", body: body, contentType: "application/json",
            authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
        let decoder = JSONDecoder()
    ...
    func deleteWorkoutItem(id: String) async throws {
        let (_, http) = try await request(
            "/workout/items/\(id)", method: "DELETE", authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
    }
    ...
    func getCustomMetricItems() async throws -> [CustomMetric] {
        ...
        throw APIError(
            statusCode: nil, message: "Unexpected custom metric response", unverified: false,
            isNetworkError: false)
    }

    func createCustomMetricItem(
        name: String, value: String, unit: String, date: String, metricType: String
    ) async throws -> CustomMetric {
        ...
        let body = try JSONSerialization.data(withJSONObject: payload)
        let (data, http) = try await request(
            "/custom-metrics/items", method: "POST", body: body, contentType: "application/json",
            authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
        let decoder = JSONDecoder()
    ...
    func deleteCustomMetricItem(id: String) async throws {
        let (_, http) = try await request(
            "/custom-metrics/items/\(id)", method: "DELETE", authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
    }
    ...
    func createGoal(title: String?, description: String, source: String?) async throws -> Goal {
        ...
        let (data, http) = try await self.request(
            "/goals", method: "POST", body: body, contentType: "application/json", authorized: true)
        guard (200..<300).contains(http.statusCode) else { throw URLError(.badServerResponse) }
